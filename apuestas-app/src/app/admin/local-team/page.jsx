...
import { getAllEquipoLocal, addEquipoLocal, deleteEquipoLocal, updateEquipoLocal } from "@/services/equipolocal.service";
...
export default function Page() {
    ...
    const fetchEquipos = async () => {
        try {
            const data = await getAllEquipoLocal();
    ...
    const handleAddEquipo = async () => {
        try {
            await addEquipoLocal({
                nombreEquipoLocal: nombreEquipo,
                representanteEquipoLocal: representanteEquipo,
                fechaFundacionLocal: fechaFundacion
            });
            setNombreEquipo('');
    ...
    const handleEditEquipo = async () => {
        try {
            await updateEquipoLocal(currentEquipo.idEquipoLocal, {
                nombreEquipoLocal: nombreEquipo,
                representanteEquipoLocal: representanteEquipo,
                fechaFundacionLocal: fechaFundacion
            });
    ...
    return (
        <div>
            <div className="flex flex-row justify-between items-center w-full">
                <h2 className="text-white text-4xl my-5">Equipos Locales</h2>
                <button
            ...
            <Table className="text-white">
                ...
                <TableBody>
                    {equipos.map((equipo) => (
                        <TableRow key={equipo.idEquipoLocal} className="hover:bg-gray-700">
                            <TableCell className="font-medium">{equipo.nombreEquipoLocal}</TableCell>
                            <TableCell className="font-medium">{equipo.representanteEquipoLocal}</TableCell>
                            <TableCell className="font-medium">{new Date(equipo.fechaFundacionLocal).toLocaleDateString()}</TableCell>
                            <TableCell className="flex flex-row justify-start items-center gap-5">
                                <button
                                    ...
                                    onClick={() => {
                                        ...
                                        setNombreEquipo(equipo.nombreEquipoLocal);
                                        setRepresentanteEquipo(equipo.representanteEquipoLocal);
                                        setFechaFundacion(new Date(equipo.fechaFundacionLocal).toISOString().substr(0, 10));
                                ...
                                <button
                                    ...
                                    onClick={() => handleDeleteEquipo(equipo.idEquipoLocal)}
                                >
                                    Eliminar
                                </button>
                            </TableCell>
